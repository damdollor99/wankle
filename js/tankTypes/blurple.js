class BlurpleTank {
	...
	update() {
		if (!STAGE_CACHE.player.dead && !this.dead) {
			...
			//blurple tanks shoot up to 5 shells rapidly
			if (this.shouldFire(ray) && this.shellDelay > 0.32 && this.shellShot < 5) {
				//it found the ray to fire upon
				this.shellShot++;
				this.shellDelay = 0;
				this.tankShock = -0.1;
				this.tank.shoot(shootCoordinates, TELE_SHELL, this.tankID);
			}
		}
